...
Sequence {
    ...
    onEnter: {
        ...
        if (result === StateBase.Finished) {
            console.log("Counter reached 10!")
        } else if (result === StateBase.Timeout) {
            console.log("Timed out waiting for counter")
        } else if (result === StateBase.Cancelled) {
...
- **`StateBase.Finished`**: Wait completed successfully
- **`StateBase.Cancelled`**: Wait was cancelled via the cancel trigger
- **`StateBase.Timeout`**: Wait timed out
- **`StateBase.Error`**: An error occurred (e.g., invalid signal name)
